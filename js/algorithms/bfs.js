...
export class BreadthFirstSearch extends BaseSearchAlgorithm {
    /**
    ...
    search(start, goal, options = {}) {
        ...
        // Queue stores [node_label, node_id, path, depth]
        const queue = [[start, startId, [start], 0]];
        const steps = [];
        let stepNumber = 0;
        ...
        while (queue.length > 0) {
            const [current, currentId, path, depth] = queue.shift();
            ...
            for (const neighbor of neighbors) {
                if (!visitedSet.has(neighbor)) {
                    ...
                    queue.push([neighbor, neighborId, [...path, neighbor], neighborDepth]);
